...
public class TaskManager {
    ...
    public static boolean markTaskAsFinished(String username, String fileResult) {
    String sql = "UPDATE tasks SET status = 'Finished', file_result = ? WHERE client_id = ?";
    try (Connection conn = DBConnection.getConnection();
         PreparedStatement ps = conn.prepareStatement(sql)) {
        ps.setString(1, fileResult); // Lưu tên file kết quả (file .txt)
